...
const SupportMenu = ({selected_root}) => {
  ...
  return (
    <div className='flex flex-row w-full justify-center items-center bg-zinc-100 gap-1 flex-wrap mb-2'>
      ...
      <Button variant='text' style={{textTransform: 'none', color: selected_root==='services'?'#ffffff':'#27272a', backgroundColor: selected_root==='services'?'#27272a':'#f4f4f5'}} size='small' onClick={()=>router.push("/services")}>Services</Button>
      <Button variant='text' style={{textTransform: 'none', color: selected_root==='shops'?'#ffffff':'#27272a', backgroundColor: selected_root==='shops'?'#27272a':'#f4f4f5'}} size='small' onClick={()=>router.push("/shops")}>Shops</Button>
