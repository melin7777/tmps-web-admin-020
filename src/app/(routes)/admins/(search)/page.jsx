...
const AdminsSearch = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(false);(async () => {
      try {
        const search_data = localStorage.getItem('admin_admins_search_data');
        if(search_data!==null){
          setSearchData(JSON.parse(search_data));
        }
        const search_nop = localStorage.getItem('admin_admins_search_nop');
        if(search_nop!==null) {
          setSearchNop(JSON.parse(search_nop));
        }
        const search_page = localStorage.getItem('admin_admins_search_page');
        if(search_page!==null) {
          setSearchPage(JSON.parse(search_page));
        }
        const sort_by = localStorage.getItem('admin_admins_search_sort_by');
        if(sort_by!==null) {
          setSearchSortBy(JSON.parse(sort_by));
        }
        const order = localStorage.getItem('admin_admins_search_order');
        if(order!==null) {
          setSearchOrder(JSON.parse(order));
        }
        const search_description = localStorage.getItem('admin_admins_search_description');
        if(search_description!==null) {
          setSearchDescription(search_description);
        }
        const search_status = localStorage.getItem('admin_admins_search_status');
        if(search_status!==null) {
          setSearchStatus(JSON.parse(search_status));
        }
        const search_rpp = localStorage.getItem('admin_admins_search_rpp');
        if(search_rpp!==null) {
          setSearchRpp(JSON.parse(search_rpp));
        }
        const search_controls_showing = localStorage.getItem('admin_admins_search_filters_showing');
        ...
        const search_selected_row = localStorage.getItem('admin_admins_search_selected_row');
        ...
        const search_scroll_top = localStorage.getItem('admin_admins_search_scroll_top');
  ...
  useEffect(() => {
    const change = () => {
      if(filtersShowing!==null){
        try {
          localStorage.setItem("admin_admins_search_filters_showing", JSON.stringify(filtersShowing));
        } 
        catch (e) {}
      }
    }
  ...
  useEffect(() => {
    const change = () => {
      if(selectedRow!==null){
        try {
          localStorage.setItem("admin_admins_search_selected_row", JSON.stringify(selectedRow));
        } 
        catch (e) {}
      }      
    }
  ...
  const editClicked = (id) => {
    if(filtersShowing!==null){
      try {
        localStorage.setItem("admin_admins_search_scroll_top", JSON.stringify(scrollTop));
  ...
  async function getSearchData(page){
    if(page>=1 && page<=searchNop){
      ...
      try{
        ...
        if(!error){
          const response = await axios.post("/api/admins/search", {
          ...
          try {
            localStorage.setItem('admin_admins_search_data', JSON.stringify(values));
            localStorage.setItem('admin_admins_search_rpp', JSON.stringify(searchRpp));
            localStorage.setItem('admin_admins_search_nop', JSON.stringify(response.data.data.nop));
            localStorage.setItem('admin_admins_search_page', JSON.stringify(page));
            localStorage.setItem('admin_admins_search_sort_by', JSON.stringify(searchSortBy));
            localStorage.setItem('admin_admins_search_order', JSON.stringify(searchOrder));
  
            localStorage.setItem('admin_admins_search_description', searchDescription);
            localStorage.setItem('admin_admins_search_status', JSON.stringify(searchStatus));
          } 
          catch (e) {
        ...
        toast.error("Admins Search Failed !", {
  ...
  return (
    <div className='form_container' style={{minHeight: (height-80)}} ref={scrollRef} onScroll={onScroll}>
      <div className='form_container_medium' style={{minHeight: (height-80)}}>
        <SupportMenu selected_root='admins'/>
        <div className='header_container'>
          <div className='header_container_left'>
            <span></span>
            <div className='header_container_left_text'>
              <span className="form_header">Admins</span>
              <span className="form_sub_header">Add or remove admins</span>
            </div>
          </div>
