...
const CategoriesSearch = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(false);(async () => {
      try {
        const search_data = localStorage.getItem('admin_categories_search_data');
        if(search_data!==null){
          setSearchData(JSON.parse(search_data));
        }
        const search_nop = localStorage.getItem('admin_categories_search_nop');
        if(search_nop!==null) {
          setSearchNop(JSON.parse(search_nop));
        }
        const search_page = localStorage.getItem('admin_categories_search_page');
        if(search_page!==null) {
          setSearchPage(JSON.parse(search_page));
        }
        const sort_by = localStorage.getItem('admin_categories_search_sort_by');
        if(sort_by!==null) {
          setSearchSortBy(JSON.parse(sort_by));
        }
        const order = localStorage.getItem('admin_categories_search_order');
        if(order!==null) {
          setSearchOrder(JSON.parse(order));
        }
        const search_description = localStorage.getItem('admin_categories_search_description');
        ...
        const search_controls_showing = localStorage.getItem('admin_categories_search_filters_showing');
        ...
        const search_selected_row = localStorage.getItem('admin_categories_search_selected_row');
  ...
  useEffect(() => {
    const change = () => {
      if(filtersShowing!==null){
        try {
          localStorage.setItem("admin_categories_search_filters_showing", JSON.stringify(filtersShowing));
        } 
        catch (e) {}
      }
    }
  ...
  useEffect(() => {
    const change = () => {
      if(selectedRow!==null){
        try {
          localStorage.setItem("admin_categories_search_selected_row", JSON.stringify(selectedRow));
        } 
        catch (e) {}
      }      
    }
    ...
    if(page>=1 && page<=searchNop){
      ...
      try{
        ...
        if(!error){
          ...
          try {
            localStorage.setItem('admin_categories_search_data', JSON.stringify(values));
            localStorage.setItem('admin_categories_search_rpp', JSON.stringify(searchRpp));
            localStorage.setItem('admin_categories_search_nop', JSON.stringify(response.data.data.nop));
            localStorage.setItem('admin_categories_search_page', JSON.stringify(page));
            localStorage.setItem('admin_categories_search_sort_by', JSON.stringify(searchSortBy));
        ...
        <div className='header_container'>
          <div className='header_container_left'>
            ...
            <div className='header_container_left_text'>
              <span className="form_header">Categories</span>
              <span className="form_sub_header">Add or remove categories</span>
            </div>
          </div>
