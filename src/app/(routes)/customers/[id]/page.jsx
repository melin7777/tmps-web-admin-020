...
const View = ({params}) => {
  ...
  useEffect(() => {
    ...
    if(params.id==="create-item"){
      setFormHeading("Create Customer");
      setFormSubHeading("Create a new customer");
    }
    else{
      setFormHeading("Edit Customer");
      setFormSubHeading("Edit an existing customer");
      loadItem(params.id);
    }
  }, []);
  ...
  const loadItem = async (id) => {
    setIsLoading(true);
    try{
      const response = await axios.post("/api/customers/find", {
        id: id
      });
      ...
      if(val.image_url==="none"){
        ...
        setPhotoURL(" http://tm-web.effisoftsolutions.com/"+val.image_url);
      }
    }
    catch(error){
  ...
  const saveClicked = async () => {
    try{
      ...
      toast.error("Customer Create Failed !", {
        position: toast.POSITION.TOP_RIGHT
      });
    }
    finally{
  ...
  const deleteImage = async () => {
    if(editId!==""){
      setIsSaving(true);
      try{
        const response = await axios.post("/api/customers/delete-image", {
          id: parseInt(editId),
        });
        setPhotoURL("none");
  ...
  const savePasswordClicked = async () => {
    ...
    if(error) {
      ...
      try{
        const response = await axios.post("/api/customers/save-password", {
          id: editId,
          password: editPassword,
        });
        setEditPassword("");
      ...
      <div className='form_container_medium' style={{minHeight: (height-80)}}>
        <div className='header_container'>
          <div className='header_container_left'>
            <IconButton onClick={()=>router.push('/customers')} sx={{backgroundColor: '#27272a', "&:hover, &.Mui-focusVisible": {backgroundColor: "#71717a"}}}><KeyboardArrowLeft sx={{width: 30, height: 30, color: '#ffffff'}}/></IconButton>
