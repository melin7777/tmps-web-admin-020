...
const FeaturesSearch = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(false);(async () => {
      try {
        const search_data = localStorage.getItem('admin_features_search_data');
        if(search_data!==null){
          setSearchData(JSON.parse(search_data));
        }
        const search_nop = localStorage.getItem('admin_features_search_nop');
        if(search_nop!==null) {
          setSearchNop(JSON.parse(search_nop));
        }
        const search_page = localStorage.getItem('admin_features_search_page');
        if(search_page!==null) {
          setSearchPage(JSON.parse(search_page));
        }
        const sort_by = localStorage.getItem('admin_features_search_sort_by');
        if(sort_by!==null) {
          setSearchSortBy(JSON.parse(sort_by));
        }
        const order = localStorage.getItem('admin_features_search_order');
        if(order!==null) {
          setSearchOrder(JSON.parse(order));
        }
        const search_description = localStorage.getItem('admin_features_search_description');
        if(search_description!==null) {
          setSearchDescription(search_description);
        }
        const search_status = localStorage.getItem('admin_features_search_status');
        if(search_status!==null) {
          setSearchStatus(JSON.parse(search_status));
        }
        const search_rpp = localStorage.getItem('admin_features_search_rpp');
        if(search_rpp!==null) {
          setSearchRpp(JSON.parse(search_rpp));
        }
        const search_controls_showing = localStorage.getItem('admin_features_search_filters_showing');
        ...
        const search_selected_row = localStorage.getItem('admin_features_search_selected_row');
        ...
        const search_scroll_top = localStorage.getItem('admin_features_search_scroll_top');
  ...
  useEffect(() => {
    const change = () => {
      if(filtersShowing!==null){
        try {
          localStorage.setItem("admin_features_search_filters_showing", JSON.stringify(filtersShowing));
        } 
        catch (e) {}
      }
    }
  ...
  useEffect(() => {
    const change = () => {
      if(selectedRow!==null){
        try {
          localStorage.setItem("admin_features_search_selected_row", JSON.stringify(selectedRow));
        } 
        catch (e) {}
      }      
    }
  ...
  const editClicked = (id) => {
    if(filtersShowing!==null){
      try {
        localStorage.setItem("admin_features_search_scroll_top", JSON.stringify(scrollTop));
  ...
  async function getSearchData(page){
    if(page>=1 && page<=searchNop){
      ...
      try{
        ...
        if(!error){
          const response = await axios.post("/api/features/search", {
          ...
          try {
            localStorage.setItem('admin_features_search_data', JSON.stringify(values));
            localStorage.setItem('admin_features_search_rpp', JSON.stringify(searchRpp));
            localStorage.setItem('admin_features_search_nop', JSON.stringify(response.data.data.nop));
            localStorage.setItem('admin_features_search_page', JSON.stringify(page));
            localStorage.setItem('admin_features_search_sort_by', JSON.stringify(searchSortBy));
            localStorage.setItem('admin_features_search_order', JSON.stringify(searchOrder));
  
            localStorage.setItem('admin_features_search_description', searchDescription);
            localStorage.setItem('admin_features_search_status', JSON.stringify(searchStatus));
          } 
          catch (e) {
        ...
        toast.error("Features Search Failed !", {
  ...
  return (
    <div className='form_container' style={{minHeight: (height-80)}} ref={scrollRef} onScroll={onScroll}>
      <div className='form_container_medium' style={{minHeight: (height-80)}}>
        <SupportMenu selected_root='features'/>
        <div className='header_container'>
          <div className='header_container_left'>
            <span></span>
            <div className='header_container_left_text'>
              <span className="form_header">Features</span>
              <span className="form_sub_header">Add or remove features</span>
            </div>
          </div>
              ...
              <div key={val.id} className='table_row'>
                ...
                <div className='table_col_start_center sm:h-[80px]'>
                  <div className='table_field_double'>
                    ...
                    <div className='table_field'>
                    <span className='table_field_label'>type:</span>
                      <span className='table_field_text_center'>{val.type}</span>
                    </div>
                  </div>
